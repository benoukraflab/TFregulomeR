...
#' commonPeaks_output <- commonPeaks(
...
commonPeaks <- function(
  ...
  if ((!missing(user_target_peak_list) && !is.list(user_target_peak_list)) || (!missing(user_compared_peak_list) && !is.list(user_compared_peak_list))) {
  ...
  if (!missing(user_target_peak_list) && length(user_target_peak_list) > 0) {
    ...
    if (missing(user_target_peak_id) || length(user_target_peak_id) != length(user_target_peak_list) || length(unique(user_target_peak_id)) != length(user_target_peak_list)) {
    ...
    for (i in seq(1, length(user_target_peak_list), 1)) {
      ...
      if (nrow(peak_i) == 0) {
        ...
        user_target_peak_id_new <- c(
          user_target_peak_id_new, user_target_peak_id[i]
        )
  ...
  if (!missing(compared_peak_id) && length(compared_peak_id) > 0) {
    ...
    for (i in compared_peak_id) {
      ...
      if (is.null(peak_i)) {
        ...
        message(
          paste0("... ... peak file loaded successfully for id '", i, "'")
        )
      }
    }
    message("... Done loading TFBS(s) from TFregulomeR")
  ...
  if (!missing(user_compared_peak_list) && length(user_compared_peak_list) > 0) {
    ...
    if (missing(user_compared_peak_id) || length(user_compared_peak_id) != length(user_compared_peak_list) || length(unique(user_compared_peak_id)) != length(user_compared_peak_list)) {
      ...
      user_compared_peak_id <- paste0(
        "user_compared_peak", seq(1, length(user_compared_peak_list), 1)
      )
    }
    ...
    for (i in seq(1, length(user_compared_peak_list), 1)) {
      ...
      if (nrow(peak_i) == 0) {
        ...
        user_compared_peak_id_new <- c(
          user_compared_peak_id_new, user_compared_peak_id[i]
        )
        peak_i_sub <- peak_i[, c(1, 2, 3)]
        colnames(peak_i_sub) <- c("chr", "start", "end")
        peak_i_sub$id <- paste0(
          "compared_peak_", as.vector(rownames(peak_i_sub))
        )
  ...
  for (i in seq(1, length(target_peak_list_all), 1)) {
    ...
    number_of_original_target <- nrow(target_peak_i)
    message(paste0("Start analyzing: ", target_id_i, "... ..."))
    ...
    if (is_taregt_TFregulome[i]) {
      ...
      if (source_i == "MethMotif") {
        ...
        meth_file_path_target <- request_content_df[
          1, c("DNA_methylation_profile")
        ]
        meth_file_200bp_path_target <- request_content_df[
          1, c("DNA_methylation_profile_200bp")
        ]
    ...
    for (j in seq(1, length(compared_peak_list_all), 1)) {
      ...
      suppressWarnings(bedTarget_with_bedcompared <- subsetByOverlaps(
        bed_target_i, bed_compared_j
      ))
      peakTarget_with_peakcompared <- unique(
        as.data.frame(bedTarget_with_bedcompared)
      )
      target_peak_with_peakcompared <- target_peak_i[which(
        target_peak_i$id %in% peakTarget_with_peakcompared$id
      ), ]
    ...
    if (isTFregulome_target) {
      motif_seq_target <- read.delim(
        motif_seq_path_target, sep = "\t", header = FALSE
      )
      if (nrow(target_peak_i) > 0) {
        # compute motif matrix
        colnames(motif_seq_target) <- c(
          "chr", "start", "end", "strand", "weight",
          "pvalue", "qvalue", "sequence"
        )
        motif_len_target <- nchar(as.character(motif_seq_target[1, "sequence"]))
        motif_seq_target$id <- paste0(
          target_id_i, "_motif_sequence_", as.vector(rownames(motif_seq_target))
        )
        motif_seq_target_grange <- GRanges(motif_seq_target$chr,
          IRanges(
            motif_seq_target$start + 1,
        ...
        suppressWarnings(motif_of_bed_target_done_common <- subsetByOverlaps(
          motif_seq_target_grange, bed_target_done_common
        ))
        motif_of_peakTarget_done_common <- unique(
          as.data.frame(motif_of_bed_target_done_common)
        )

        if (nrow(motif_of_peakTarget_done_common) > 0) {
          # nPeaks
          suppressWarnings(peaks_with_motif_gr <- subsetByOverlaps(
            bed_target_done_common, motif_seq_target_grange
          ))
          peaks_with_motif_df <- unique(as.data.frame(peaks_with_motif_gr))
          number_of_target_peak_i_with_motif <- nrow(peaks_with_motif_df)


          motif_of_peakTarget_done_common_allInfo <- motif_seq_target[which(
            motif_seq_target$id %in% motif_of_peakTarget_done_common$id
          ), ]
          motif_matrix_of_peakTarget_done_common <- formMatrixFromSeq(input_sequence = as.vector(motif_of_peakTarget_done_common_allInfo$sequence), motif_format = motif_type)
          # compute beta score matrix
          if (isMethMotifID_target) {
            # calculate beta score matrix
            # methylation file can be empty
            meth_level_target <- tryCatch(read.delim(meth_file_path_target, sep = "\t", header = FALSE), error = function(e) data.frame())
            # methylation file can be empty
            if (nrow(meth_level_target) == 0) {
              beta_score_matrix_of_peakTarget_done_common <- formBetaScoreFromSeq(input_meth = data.frame(), WGBS_replicate = WGBS_replicate_target, motif_len = motif_len_target)
            } else {
              colnames(meth_level_target) <- c(
                "chr", "start", "end", "meth_score", "C_num", "T_num", "seq_chr", "seq_start",
              ...
              meth_level_target_grange <- GRanges(meth_level_target$seq_chr,
                IRanges(
                  meth_level_target$seq_start,
                  meth_level_target$seq_end
                ),
                id = meth_level_target$id
              )
              suppressWarnings(meth_level_peakTarget_done_common <- unique(as.data.frame(subsetByOverlaps(meth_level_target_grange, motif_of_bed_target_done_common))))
              ...
              beta_score_matrix_of_peakTarget_done_common <- formBetaScoreFromSeq(input_meth = meth_level_peakTarget_done_common_allInfo, WGBS_replicate = WGBS_replicate_target, motif_len = motif_len_target)
            }
          } else {
        ...
        if (methylation_profile_in_narrow_region) {
          ...
          if (isMethMotifID_target) {
            meth_level_200bp_target <- tryCatch(read.delim(meth_file_200bp_path_target, sep = "\t", header = FALSE), error = function(e) data.frame())
            if (nrow(meth_level_200bp_target) == 0) {
              meth_score_distri_target <- formBetaScoreDistri(input_meth = data.frame())
            } else {
              colnames(meth_level_200bp_target) <- c(
              ...
              suppressWarnings(
                meth_level_in_common_peaks_200bp <- unique(as.data.frame(
                  subsetByOverlaps(meth_level_200bp_target_grange, bed_target_i)
                ))
              )
              meth_level_in_common_peaks_200bp_allInfo <- unique(meth_level_200bp_target[which(meth_level_200bp_target$id %in% meth_level_in_common_peaks_200bp$id), ])
              meth_score_distri_target <- formBetaScoreDistri(
                input_meth = as.data.frame(
                  meth_level_in_common_peaks_200bp_allInfo$meth_score
                )
              )
            }
          }
    ...
    new_CommonPeaksMM <- updateCommonPeaksMM(
      theObject = new_CommonPeaksMM,
      id = paste0(target_id_i, "_common_peaks"),
      common_percentage = 100 * nrow(target_peak_i) / number_of_original_target,
...
formBetaScoreDistri <- function(input_meth) {
  ...
  if (nrow(input_meth) > 0) {
    ...
    betascore_hist <- hist(
      input_meth$beta_score,
      breaks = c(seq(0, 100, 10)), plot = FALSE
    )
    beta_score_distri <- c(betascore_hist$counts)
  }
  names(beta_score_distri) <- c(
