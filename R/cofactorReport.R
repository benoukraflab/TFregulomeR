...
cofactorReport <- function(
  ...
  for (i in seq(1, nrow(intersectPeakMatrix), 1)) {
    ...
    intersect_matrix_filter_i <- intersect_matrix_t_i[
      (intersect_matrix_t_i[, 1] >= cobinding_threshold * 100), , drop = FALSE
    ]
    ...
    intersect_matrix_order_i <- intersect_matrix_filter_i[
      order(intersect_matrix_filter_i[,1]), , drop = FALSE
    ]
    if (nrow(intersect_matrix_filter_i) > top_num) {
      ...
      intersect_matrix_order_i <- intersect_matrix_order_i[
        seq(
          nrow(intersect_matrix_filter_i) - top_num + 1,
          nrow(intersect_matrix_filter_i), 1
        ), , drop = FALSE
      ]
    }
    # cobinding heatmap
    intersect_matrix_heatmap_i <- as.data.frame(
      matrix(nrow = nrow(intersect_matrix_order_i), ncol = 5)
    )
    colnames(intersect_matrix_heatmap_i) <- c(
      "x", "new_x", "y", "new_y", "value"
    )
    ...
    intersect_matrix_heatmap_i$new_x <- unlist(
      lapply(
    ...
    intersect_matrix_heatmap_i$new_y <- factor(
    ...
    p1 <- ggplot2::ggplot(
      ...
      ggplot2::aes(
        x = intersect_matrix_heatmap_i$new_x,
    ...
    for (j in order(-seq(1, nrow(intersect_matrix_heatmap_i), 1))) {
      ...
      motif_j <- t(
        intersectPeakMatrix_i[x_j, y_j][[1]]@MethMotif_x@MMmotif@motif_matrix
      )
      top <- 10
      bottom <- 0
      if (j == 1) {
      ...
      p_j <- ggplot2::ggplot() +
        ggplot2::geom_logo(data = motif_j, method = "bits") +
        ggplot2::theme(
    ...
    if (!is.na(
      intersectPeakMatrix_i[1, 1][[1]]@MethMotif_x@MMBetaScore[1, 1]
    )) {
      methylation_matrix_inside <- as.data.frame(
        matrix(nrow = nrow(intersect_matrix_order_i) * 3, ncol = 5)
      )
      colnames(methylation_matrix_inside) <- c(
        "x", "y", "new_y", "meth_interval", "value"
      )
      methylation_matrix_inside$x <- colnames(intersect_matrix_order_i)
      methylation_matrix_inside$y <- unlist(
        lapply(rownames(intersect_matrix_order_i), function(x) rep(x, 3))
      )
      methylation_matrix_inside$new_y <- paste(
        unlist(
          lapply(
            methylation_matrix_inside$y,
            function(x) tail(unlist(strsplit(x, split = "_")), 1)
          )
        ),
        rep(rev(rownames(intersect_matrix_heatmap_i)), each = 3), sep = "-"
      )

      for (j in seq(1, nrow(intersect_matrix_heatmap_i), 1)) {
        x_j <- intersect_matrix_heatmap_i$x[j]
        y_j <- intersect_matrix_heatmap_i$y[j]
        meth_matrix <- intersectPeakMatrix_i[
          x_j, y_j
        ][[1]]@MethMotif_x@MMBetaScore
        if (sum(meth_matrix) > 0) {
          ...
          methylation_matrix_inside[(
            methylation_matrix_inside$x == x_j &
              methylation_matrix_inside$y == y_j
      ...
      methylation_matrix_inside$new_y <- factor(
        methylation_matrix_inside$new_y,
        levels = as.character(intersect_matrix_heatmap_i$new_y)
      )

      methylation_matrix_inside$meth_interval <- factor(
        methylation_matrix_inside$meth_interval,
      ...
      p3 <- ggplot2::ggplot(
        methylation_matrix_inside,
        ggplot2::aes(
          x = methylation_matrix_inside$meth_interval,
    ...
    if (!is.na(intersectPeakMatrix_i[1, 1][[1]]@methylation_profile_x[1, 1])) {
      methylation_matrix <- as.data.frame(
        matrix(nrow = nrow(intersect_matrix_order_i) * 3, ncol = 5)
      )
      colnames(methylation_matrix) <- c(
        "x", "y", "new_y", "meth_interval", "value"
      )
      methylation_matrix$x <- colnames(intersect_matrix_order_i)
      methylation_matrix$y <- unlist(
        lapply(rownames(intersect_matrix_order_i), function(x) rep(x, 3))
      )
      methylation_matrix$new_y <- paste(unlist(
        lapply(
          methylation_matrix$y,
          function(x) tail(unlist(strsplit(x, split = "_")), 1)
        )
      ), rep(rev(rownames(intersect_matrix_heatmap_i)), each = 3), sep = "-")
      ...
      for (j in seq(1, nrow(intersect_matrix_order_i), 1)) {
        ...
        if (sum(meth_matrix) > 0) {
          ...
          methylation_matrix[(
            methylation_matrix$x == x_j &
              methylation_matrix$y == y_j
          ), "meth_interval"] <- c("unmeth", "in-between", "meth")
          methylation_matrix[(
            methylation_matrix$x == x_j &
              methylation_matrix$y == y_j
      ...
      methylation_matrix$new_y <- factor(
        methylation_matrix$new_y,
        levels = as.character(intersect_matrix_heatmap_i$new_y)
      )

      methylation_matrix$meth_interval <- factor(
        methylation_matrix$meth_interval,
        levels = as.character(c("unmeth", "in-between", "meth"))
      )
      p4 <- ggplot2::ggplot(
        methylation_matrix,
        ggplot2::aes(
          x = methylation_matrix$meth_interval,
    ...
    # methylation legend
    p_legend2 <- ggplot2::ggplot(legend1_matrix,
      ggplot2::aes(
        x = legend1_matrix$value,
        y = legend1_matrix$cobinding,
        fill = legend1_matrix$value
    ...
    suppressMessages(tag_median_res_i <- intersectPeakMatrixResult(
      intersectPeakMatrix_i,
    ...
    tag_median_i_t_filter <- tag_median_i_t[
    ...
    suppressMessages(tag_q1_res_i <- intersectPeakMatrixResult(
      intersectPeakMatrix_i,
      return_tag_density = TRUE,
    ...
    tag_q1_i_t_filter <- tag_q1_i_t[intersect_matrix_heatmap_i$y, unique(
      intersect_matrix_heatmap_i$x
    ), drop = FALSE]
    colnames(tag_q1_i_t_filter) <- "y"
    tag_q1_i_t_filter$x <- seq(1, nrow(tag_q1_i_t_filter), 1)

    suppressMessages(tag_q3_res_i <- intersectPeakMatrixResult(
      intersectPeakMatrix_i,
      return_tag_density = TRUE,
    ...
    tag_q3_i_t_filter <- tag_q3_i_t[intersect_matrix_heatmap_i$y, unique(
      intersect_matrix_heatmap_i$x
    ), drop = FALSE]
    ...
    tag_x_max <- nrow(tag_median_i_t_filter) + tag_x_max_list[
      nrow(tag_median_i_t_filter)
    ]
    ...
    p_tag <- ggplot2::ggplot(
      tag_median_i_t_filter,
      ggplot2::aes(x = tag_median_i_t_filter$x, y = tag_median_i_t_filter$y)
    ) +
      ...
      ggplot2::geom_point(
        data = tag_quartile_total1, shape = "|",
        size = 3,
        mapping = ggplot2::aes(
          x = tag_quartile_total1$x, y = tag_quartile_total1$y
        )
      ) +
      ggplot2::geom_segment(
        data = tag_quartile_total2,
        mapping = ggplot2::aes(
          x = tag_quartile_total2$x1,
